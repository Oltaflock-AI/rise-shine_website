...
import { BadgeCheck, BedDouble, CalendarDays, Loader2, MapPin, Printer, ShieldCheck, TriangleAlert, Users } from "lucide-react";
...
export function HotelVoucherView({ bookingId }: { bookingId: number }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/hotels/booking-detail", {
      ...
      .catch(() => alive && setDetail({ ok: false, error: "Could not load this booking." }))
    ...
    return (
      <div className="grid min-h-[60vh] place-items-center px-6">
        <span className="inline-flex items-center gap-2 text-muted">
          <Loader2 className="animate-spin text-red" size={18} aria-hidden /> Loading your voucher…
        </span>
      </div>
    );
  }
    ...
    return (
      <Container>
        <div className="mx-auto my-16 max-w-lg rounded-brand-lg border border-line bg-white p-8 text-center shadow-brand-sm">
          ...
          <p className="mb-6 text-muted">{detail?.error || "Please try again, or contact us and we'll send it over."}</p>
    ...
    <Container>
      <div className="mx-auto my-12 max-w-3xl rounded-brand-lg border border-line bg-white p-8 shadow-brand-sm print:border-0 print:shadow-none">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4 border-b border-line pb-6">
          <div>
            <p className="text-[0.72rem] font-bold uppercase tracking-[0.14em] text-red">Hotel voucher</p>
            ...
            {(detail.address || detail.city) && (
              <p className="mt-1 flex items-start gap-1.5 text-[0.88rem] text-muted">
                <MapPin size={14} className="mt-0.5 flex-none text-red" aria-hidden />
        ...
        <dl className="mb-6 grid gap-3 sm:grid-cols-2">
          <div>
            <dt className="text-[0.75rem] font-semibold uppercase tracking-wide text-muted">Confirmation no.</dt>
            <dd className="text-[0.95rem] font-bold tracking-wide text-navy">{detail.confirmationNo || "—"}</dd>
          </div>
          <div>
            <dt className="text-[0.75rem] font-semibold uppercase tracking-wide text-muted">Booking reference</dt>
            <dd className="text-[0.95rem] font-semibold text-ink">{detail.bookingRefNo || detail.bookingId || "—"}</dd>
          </div>
          <div>
            <dt className="text-[0.75rem] font-semibold uppercase tracking-wide text-muted">Stay</dt>
            <dd className="inline-flex items-center gap-1.5 text-[0.95rem] font-semibold text-ink">
              ...
              {formatDate(toISO(detail.checkIn))} → {formatDate(toISO(detail.checkOut))}
            </dd>
          </div>
          <div>
            <dt className="text-[0.75rem] font-semibold uppercase tracking-wide text-muted">Rooms</dt>
            <dd className="inline-flex items-center gap-1.5 text-[0.95rem] font-semibold text-ink">
          ...
          <div>
            <dt className="text-[0.75rem] font-semibold uppercase tracking-wide text-muted">Booking amount</dt>
            <dd className="text-[0.95rem] font-bold text-navy">{money(detail.invoiceAmount, currency)}</dd>
          </div>
          <div>
            <dt className="text-[0.75rem] font-semibold uppercase tracking-wide text-muted">Voucher</dt>
        ...
        {(detail.rooms ?? []).map((room, i) => (
          <div key={i} className="mb-4 rounded-brand border border-line p-5">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[0.84rem] text-muted">
              ...
              {room.totalFare != null && (
                <span className="font-semibold text-ink">{money(room.totalFare, room.currency || currency)}</span>
              )}
            </div>
            {room.guests.length > 0 && (
              <p className="mt-2 flex items-start gap-1.5 text-[0.84rem] text-ink">
                <Users size={14} className="mt-0.5 flex-none text-red" aria-hidden />
                {room.guests.join(", ")}
              </p>
            )}
            {room.cancelPolicies.length > 0 && (
              <div className="mt-3 border-t border-line pt-3 text-[0.82rem]">
                <p className="mb-1 inline-flex items-center gap-1.5 font-semibold text-ink">
                  <ShieldCheck size={14} className="text-red" aria-hidden /> Cancellation policy
                </p>
                <ul className="list-disc pl-5 text-muted">
                  {room.cancelPolicies.map((p, k) => (
                    <li key={k}>
                      From {formatDate(toISO(p.fromDate)) || p.fromDate} (UTC): {chargeLabel(p, room.currency || currency)}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-line pt-6 print:hidden">
          <button
            ...
            className="inline-flex min-h-11 items-center gap-2 rounded-full border border-line px-5 text-[0.85rem] font-semibold text-ink hover:border-red/50"
          >
            <Printer size={15} aria-hidden /> Print voucher
          </button>
          <Link href="/account" className="text-[0.85rem] font-semibold text-red hover:underline">
            My bookings
          </Link>
        </div>
      </div>
